        ...
        {photos.map((photo, index) => (
          <figure key={photo.src} className="group relative mb-4 break-inside-avoid">
            ...
            {photo.caption && (
              ...
              <figcaption className="pointer-events-none absolute inset-x-0 bottom-0 rounded-b-lg bg-gradient-to-t from-black/75 via-black/20 to-transparent p-3 text-sm font-medium text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100 [@media(hover:none)]:opacity-100">
                {photo.caption}
              </figcaption>
            )}
          </figure>
